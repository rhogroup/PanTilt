 ...
 * Note: SOU (June/2009)
 * 
 * The BiclopsJointConfiguration Service connects to the Biclops service
...
namespace UMass.LPR.BiclopsJointConfiguration
{
	[Contract(Contract.Identifier)]
    [AlternateContract(jointconfig.Contract.Identifier)]
	[DisplayName("BiclopsJointConfiguration")]
	[Description("BiclopsJointConfiguration service")]
	class BiclopsJointConfigurationService : DsspServiceBase
	{
        /// <summary>
        /// _state
        /// </summary>
        [InitialStatePartner(Optional = true, ServiceUri = ServicePaths.Store+"\\biclopsjointconfiguration.xml")]
        ...
        /// <summary>
        /// </summary>
        [ServicePort("/biclopsjointconfiguration", AllowMultipleInstances = false)]
        ...
        [EmbeddedResource("UMass.LPR.BiclopsJointConfiguration.BiclopsJointConfiguration.xslt")]
		...
		public BiclopsJointConfigurationService(DsspServiceCreationPort creationPort)
			: base(creationPort)
		{
		}
		...
		protected override void Start()
		{
            ...
            if (_state == null)
            {
                ...
                LogInfo("Creating new BiClopsJointConfiguration State...");

                // get state
                _state = new jointconfig.JointConfigurationState();

                _state.NumLinks = 7; // this'll need to get updated accordingly
                _state.NumJoints = 2;
            ...
            Console.WriteLine("Starting Biclops JointConfiguration Thread");
        ...
        public IEnumerator<ITask> UpdateStateHandler(jointconfig.UpdateState update)
        {
             ...
             */
            //use DESIRED instead of ACTUAL
            _state.JointAngles.dat[0] = _state.DesiredJointAngles.dat[0];
            _state.JointAngles.dat[1] = _state.DesiredJointAngles.dat[1];
        ...
        public virtual IEnumerator<ITask> HttpPostHandler(HttpPost httpPost)
        {
            ...
            {
                Console.WriteLine("BiclopsJointConfiguration::Home()");
            ...
            {
                Console.WriteLine("BiclopsJointConfiguration::Park()");
        ...
        #region Connect to BiclopsService
        ...
        public IEnumerator<ITask> ConnectToBiclopsService()
        {
            ...
            while (_biclopsConnected == false)
            {
                for (k = 0; k < _remoteHosts.Count; k++)
                {
                    ...
                    {
                        Console.WriteLine("BiclopsJointConfiguration: cannot to connect to the Biclops Service\nStart one now if it has not been started and add the host/port to the hosts.txt");
                        Thread.Sleep(2000);
                    }
                }
