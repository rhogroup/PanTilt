        ...
        if (comm->IsOpen()) {
            ...
            if (comm->Open()) {
                ...
                if (Sync() != PMD_ERR_OK) {
                    ...
                    case 115200: trialBaud = 250000; break;
                    case 250000: trialBaud = 416667; break;
                    case 416667: trialBaud =   9600; break;
                    }
                    trialParity = desiredParity;
                    trialStopBits = desiredStopBits;
                    comm->Close();
...
//                } else
        ...
        if (status != PMD_ERR_OK) 
		{
            cout << "comm failure on controller " << (int)xbuf[0] 
                 << ", status " << hex << status << dec << endl;
			//comm->FlushRecv();
			ReSync();
		}
        commandAttempts++;
...
//    throw status;   --RRB removed for debugging
	return status; //--RRB added for debugging
