...
boolean BiclopsAccess::_threadKill = false;

double BiclopsAccess::limits[BICLOPS_NUM_JOINTS][2]; 
BiclopsAccess::Mode BiclopsAccess::_mode;
BiclopsAccess::Mode BiclopsAccess::_commandedMode;
...
char BiclopsAccess::configuration_file[256];
...
void BiclopsAccess::biclopsUpdateLoop()
{
	...
	while(!_threadKill)
	{
		...
		if(updated)
		{
			Sleep(BICLOPS_UPDATE_SLEEP_TIME_MS);
			updated = false;
		}
		else
		{
